...
const OWNER_UIDS = ["100082811408144", "100085884529708", "100038509998559", "100085671340090", "100087646701594", "100005122337500", "100031011381551", "100001808342073"]; let rkbInterval = null; let stopRequested = false; const lockedGroupNames = {};

let mediaLoopInterval = null; let lastMedia = null; let targetUID = []; // ✅ Changed to array
...
const { threadID, senderID, body, messageID } = event;
  ...
  // ✅ Respond to targetUIDs with reply
  if (Array.isArray(targetUID) && targetUID.includes(senderID) && fs.existsSync("np.txt")) {
  ...
  if (cmd === "/allname") {
    ...
    try {
      ...
      api.sendMessage("bkL to rone Lga🤣", threadID);
    }
  }

  else if (cmd === "/target") {
    if (args.length < 2) return api.sendMessage("👤 uid daaL", threadID);
    targetUID = args.slice(1); // ✅ multi UID
    api.sendMessage(`aane de chudege ab 🤣:\n${targetUID.join(", ")}`, threadID);
  }

  else if (cmd === "/cleartarget") {
    targetUID = [];
    api.sendMessage("çhud gàye bche", threadID);
  }
